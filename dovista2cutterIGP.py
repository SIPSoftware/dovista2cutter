...
parser.add_argument("-i", dest="inputfile", help="plik wejściowy")
parser.add_argument("-o", dest="outputfile", help="plik wyjściowy")
parser.add_argument(
    "--sepfn",
    action="extend",
    nargs="+",
    type=str,
    dest="separate_order_factory_number",
    help="lista factoryNumber dla których będzie podział zlecenia ze wzgledu na Elevation Shape",
)
parser.add_argument(
    "--config-file", dest="config_file_path", help="ścieżka do pliku konfiguracyjnego"
)
...
for order_line_node in root.findall("cac:OrderLine", ns):
    ...
    for line_item_node in order_line_node.findall("cac:LineItem", ns):
        ...
        for additional_item_property_node in line_item_node.findall(
            "./cac:Item/cac:AdditionalItemProperty", ns
        ):
        ...
        for additional_item_property_node in line_item_node.findall(
            ...
            additional_properties2[name] = {"name": name, "value": value}
        ...
        glass_long_text = getNodeValue(
            line_item_node,
            ns,
            "./cac:Item/cac:AdditionalItemProperty/cbc:Name[.='Glass Long Text']/../cbc:Value",
        )
        ...
        finished_product_shape = getAdditionalPropertiesValue(
            {"additional_properties": additional_properties2},
        ...
        dva_order_string = "_".join(
            [factoryNumber[0:2], finished_product_shape[0:2], platform[0:2]]
        ).upper()
        # dva_order_string = '_'.join([factoryNumber[0:2],platform[0:2]]).upper()
        if (
            args.separate_order_factory_number
            and factoryNumber in args.separate_order_factory_number
        ):
        ...
        order_positions.append(
            {
                ...
                "buyers_item_identification": getNodeValue(
                    line_item_node, ns, "./cac:Item/cac:BuyersItemIdentification/cbc:ID"
                ),
                "quantity": dovista_float2int(
                    getNodeValue(line_item_node, ns, "./cbc:Quantity")
                ),
...
xml_import = ET.Element(
    "cutter_import_order_file",
    attrib={
        "xmlns:xsi": xmlns_xsi,
        "xsi:noNamespaceSchemaLocation": xsi,
        "format_version": COIFVERSION,
        "script": "dovista2cutter",
        "script_version": BUILDVERSION,
    },
)
...
for k in orders.keys():
    ...
    ET.SubElement(
        xml_order, "additional_info", attrib={"type": "61", "comment": "C_VENDOR"}
    ).text = orders[k]["vendor"]
    ...
    ET.SubElement(xml_address, "address", {"info": orders[k]["delivery_info"]}).text = (
        deliveryAddress[:-1]
    )

    ET.SubElement(xml_address, "city").text = deliveryCity
    ET.SubElement(
        xml_order, "customer_name", {"comment": "factory=" + factoryNumber}
    ).text = customer_name
    ...
    ET.SubElement(xml_order, "internal_number").text = orders[k]["internal_order"]
    ...
    for position in positions:
        ...
        ET.SubElement(xml_position, "width").text = str(
            dovista_int2int(
                getAdditionalPropertiesValue(position, "C_GLASS_WIDTH", "value")
            )
        )
        ET.SubElement(xml_position, "height").text = str(
            dovista_int2int(
        ...
        ET.SubElement(xml_position, "structure_code").text = (
            getAdditionalPropertiesValue(position, "C_GLASS_CODE", "name")
        )
        ET.SubElement(xml_position, "structure_code2").text = (
            getAdditionalPropertiesValue(position, "C_GLASS_CODE", "value")
        )
        ...
        gas1 = str(
            getAdditionalPropertiesValue(position, "C_GLASS_AIR_SPACER1", "name")
        )
        ...
        gas2 = str(
            getAdditionalPropertiesValue(position, "C_GLASS_AIR_SPACER2", "name")
        )
        ...
        gas3 = str(
            getAdditionalPropertiesValue(position, "C_GLASS_AIR_SPACER3", "name")
        )
        ...
        gas4 = str(
            getAdditionalPropertiesValue(position, "C_GLASS_AIR_SPACER4", "name")
        )
        ...
        if spacer1 != "" and spacer1 != "0":
            ET.SubElement(
                xml_product, "spacer1", {"thickness": frame1_thickness}
            ).text = spacer1
        ...
        if spacer2 != "" and spacer2 != "0":
            ET.SubElement(
                xml_product, "spacer2", {"thickness": frame2_thickness}
            ).text = spacer2
        ...
        if spacer3 != "" and spacer3 != "0":
            ET.SubElement(
                xml_product, "spacer3", {"thickness": frame3_thickness}
            ).text = spacer3
        ...
        if spacer4 != "" and spacer4 != "0":
            ET.SubElement(
                xml_product, "spacer4", {"thickness": frame4_thickness}
            ).text = spacer4
        ...
        ET.SubElement(xml_position, "additional_description").text = (
            orderNumberByCustomer + str(position["order_position"])
        )

        # --- BEGIN --- obsługa kształtów DOVISTA
        shape_drawing = getAdditionalPropertiesValue(position, "C_DRAWING", "value")
        shape_elevation = getAdditionalPropertiesValue(
            position, "C_ELEVATION_SHAPE", "name"
        )
        shape_params_def = {
            "l": "C_W",
            "l1": "C_W1",
        ...
        if isGlazingBar:
                ...
                for frameno in range(1, chamber_count + 1):
                    ...
                    for k in d.keys():
                        ...
                        if value > 0 and k[0] == "w":
                            ...
                            bar_no_w = bar_no_w + 1
                            ET.SubElement(
                                xml_gb_frame_dim, k[0] + str(bar_no_w)
                            ).text = str(value)
                    # analiza listew poziomych (od góry)
                    bar_no_h = 0
                    height = dovista_int2int(
                        getAdditionalPropertiesValue(
                            position, "C_GLASS_HEIGHT", "value"
                        )
                    )
                    for k in d.keys():
                        ...
                        if value > 0 and k[0] == "h":
                            ...
                            bar_no_h = bar_no_h + 1
                            ET.SubElement(
                                xml_gb_frame_dim, k[0] + str(bar_no_h)
                            ).text = str(value)
            ...
            # other than grid glazingbar
            else:
                xml_gb = ET.SubElement(xml_position, "custom_glazing_bar")
                for frameno in range(1, chamber_count + 1):
                    xml_gb_frame = ET.SubElement(xml_gb, "frame", {"no": str(frameno)})
                    xml_gb_frame_mat = ET.SubElement(xml_gb_frame, "material")
                    material = getAdditionalPropertiesValue(
                        position, "C_GLZBAR_G_VAR", "name"
                    )
                    ET.SubElement(xml_gb_frame_mat, "w1").text = material
        ...
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "1", "comment": "Purchase order position "},
        ).text = str(position["order_position"])
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "5", "comment": "OrderLine.Document Reference "},
        ).text = str(position["document_reference"]).split("/")[0]
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "6", "comment": "OrderLine.Document Reference "},
        ).text = str(position["document_reference"]).split("/")[1]
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "27", "comment": "SellersItemIdentification"},
        ).text = str(position["vendor_info"])
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "57", "comment": "C_GLASS_CODE.value"},
        ).text = getAdditionalPropertiesValue(position, "C_GLASS_CODE", "value")
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "176", "comment": "Customer"},
        ).text = customer_brand
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "207", "comment": "Glass Long Text"},
        ).text = getAdditionalPropertiesValue(
            {"additional_properties": position["additional_properties2"]},
            "Glass Long Text",
            "value",
        )
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "211", "comment": "Delivery address"},
        ).text = deliveryAddress
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "226", "comment": "C_DRAWING"},
        ).text = str(getAdditionalPropertiesValue(position, "C_DRAWING", "value"))
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "227", "comment": "C_FIELD_ELEVATION"},
        ).text = str(getAdditionalPropertiesValue(position, "C_GLZBAR_G_ELEV", "name"))
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "228", "comment": "C_GLZBAR_G_VAR"},
        ).text = str(getAdditionalPropertiesValue(position, "C_GLZBAR_G_VAR", "value"))
        ET.SubElement(
            xml_position,
            "additional_info",
        ...
        ET.SubElement(
            xml_position,
            "additional_info",
            attrib={"type": "906", "comment": "Finished Product Shape.value"},
        ).text = str(
            getAdditionalPropertiesValue(
                {"additional_properties": position["additional_properties2"]},
                "Finished Product Shape",
...
xmlstr = minidom.parseString(ET.tostring(xml_import, encoding="utf-8")).toprettyxml(
    indent="   "
)
# print(xmlstr)
