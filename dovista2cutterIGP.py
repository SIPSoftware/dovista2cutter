...
parser = argparse.ArgumentParser(description='Skrypt konwersji pliku Dovista XML (wersja IGP) do formatu Cutter Customer Order Import File')
...
deliveryCity = getNodeValue(root,ns,'./cac:Delivery/cac:DeliveryParty/cac:PostalAddress/cbc:StreetName')
...
for order_line_node in root.findall('cac:OrderLine',ns):
    ...
    document_reference = getNodeValue(order_line_node,ns,'./cac:DocumentReference/cbc:ID')
    for line_item_node in order_line_node.findall('cac:LineItem',ns):
        ...
        glass_long_text = getNodeValue(line_item_node,ns,"./cac:Item/cac:AdditionalItemProperty/cbc:Name[.='Glass Long Text']/../cbc:Value")
...
xml_import = ET.Element('cutter_import_order_file',attrib={"xmlns:xsi":xmlns_xsi,"xsi:noNamespaceSchemaLocation":xsi,"format_version":COIFVERSION,"script":"dovista2cutter","script_version":BUILDVERSION})
...
#generowanie gałezi <order>
    ...
    for position in positions:
        ...
        ET.SubElement(xml_position,'structure_name').text = joinStringsWithoutEmpty(elements,'/')
        ET.SubElement(xml_position,'structure_code').text = getAdditionalPropertiesValue(position,'C_GLASS_CODE','name')
        ET.SubElement(xml_position,'structure_code2').text = getAdditionalPropertiesValue(position,'C_GLASS_CODE','value')
        ...
        ET.SubElement(xml_position,'additional_description').text = orderNumberByCustomer + str(position['order_position'])
        ...
        ET.SubElement(xml_position,'additional_info',attrib={"type": "1", "comment":"Purchase order position "}).text = str(position['order_position'])
        ET.SubElement(xml_position,'additional_info',attrib={"type": "5", "comment":"OrderLine.Document Reference "}).text = str(position['document_reference']).split('/')[0]
        ET.SubElement(xml_position,'additional_info',attrib={"type": "6", "comment":"OrderLine.Document Reference "}).text = str(position['document_reference']).split('/')[1]
        ET.SubElement(xml_position,'additional_info',attrib={"type": "27", "comment":"SellersItemIdentification"}).text = str(position['vendor_info'])
        ET.SubElement(xml_position,'additional_info',attrib={"type": "57", "comment":"C_GLASS_CODE.value"}).text = getAdditionalPropertiesValue(position,'C_GLASS_CODE','value')
        ET.SubElement(xml_position,'additional_info',attrib={"type": "176", "comment":"Customer"}).text = customer_brand
        ET.SubElement(xml_position,'additional_info',attrib={"type": "207", "comment":"Glass Long Text"}).text = glass_long_text
        ET.SubElement(xml_position,'additional_info',attrib={"type": "211", "comment":"Delivery address"}).text = deliveryAddress
        ET.SubElement(xml_position,'additional_info',attrib={"type": "226", "comment":"C_DRAWING"}).text = str(getAdditionalPropertiesValue(position,'C_DRAWING','value'))
        ET.SubElement(xml_position,'additional_info',attrib={"type": "227", "comment":"C_FIELD_ELEVATION"}).text = str(getAdditionalPropertiesValue(position,'C_GLZBAR_G_ELEV','name'))
