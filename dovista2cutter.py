...
parser = argparse.ArgumentParser(
    description='Skrypt konwersji pliku Dovista XML do formatu Cutter Customer Order Import File')
    ...
    for line_item_node in order_line_node.findall('cac:LineItem', ns):
        ...
        if args.separate_order_factory_number:
            if factoryNumber == 'T3':
                ...
                order_note = 'Strefa montażu {0} {1}! NIE ŁĄCZYĆ Z SZBAMI INNYCH STREF DVA !'.format(
                    factoryNumber, elevation_shape)
            if factoryNumber == 'T7':
...
xml_import = ET.Element('cutterImportOrderFile', attrib={
                        "format_version": COIFVERSION, "script": "dovista2cutter", "script_version": BUILDVERSION})
...
for k in orders.keys():
    ...
    ET.SubElement(xml_order, 'additionalInfo', attrib={"type": "301", "comment": "OrderConfirmation: BuyerCustomerParty/ID"}
                  ).text = getNodeValue(root, ns, './cac:BuyerCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID')
    ET.SubElement(xml_order, 'additionalInfo', attrib={"type": "302", "comment": "OrderConfirmation: BuyerCustomerParty/CompanyID"}
                  ).text = getNodeValue(root, ns, './cac:BuyerCustomerParty/cac:Party/cac:PartyLegalEntity/cbc:CompanyID')
    ...
    if customer_brand in config_cutter_customer_info:
        if 'cutter_number' in config_cutter_customer_info[customer_brand]:
            ET.SubElement(xml_order, 'customer_cutter_number').text = str(
                config_cutter_customer_info[customer_brand]['cutter_number'])
        if 'delivery_adresses' in config_cutter_customer_info[customer_brand]:
            if factoryNumber in config_cutter_customer_info[customer_brand]['delivery_adresses']:
                ET.SubElement(xml_order, 'delivery_cutter_number').text = str(
                    config_cutter_customer_info[customer_brand]['delivery_adresses'][factoryNumber])
    ...
    for position in positions:
        ...
        ET.SubElement(xml_position, 'structureName').text = getAdditionalPropertiesValue(
            position, 'C_GLASS_CODE', 'value')

        ET.SubElement(xml_position, 'description').text = str(position['vendor_info']).replace(
            'GLASS_', '').replace('-', '').replace(' ', '').replace('(', '').replace(')', '')
        ET.SubElement(
        ...
        if len(getAdditionalPropertiesValue(position, 'C_GLASS_SEQ_W1', 'value')) > 0:
            ...
            for frameno in range(1, chamber_count+1):
                ...
                gb_colour = getAdditionalPropertiesValue(
                    position, 'C_GLASS_SPACER'+str(frameno)+'_COLOUR', 'value')
                for k in d.keys():
                    value = getAdditionalPropertiesValue(
                        position, d[k], 'name')
                    if value != '' and value != '00':
                        ET.SubElement(xml_gb_frame_mat,
                                      k).text = value + ' '+gb_colour

        # --- END --- obsługa szprosów dowolnych

        # generowanie zlec_typ dla pozycji
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "301", "comment": "Label/Frame: DVA Vendor number"}).text = str(getAdditionalPropertiesValue(position, 'C_VENDOR', 'value'))
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "303", "comment": "Label: DVA Purchase order number"}).text = orderNumberByCustomer
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "304", "comment": "Label: DVA Purchase order position "}).text = str(position['order_position'])
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "305", "comment": "Label/Frame: DVA Sales order"}).text = str(position['sales_order'])
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "306", "comment": "Label: DVA Vendor info"}).text = str(position['vendor_info'])
        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "307", "comment": "Label: DVA Platform"}).text = str(
            getAdditionalPropertiesValue(position, 'C_PLATFORM', 'value'))
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "308", "comment": "Label: DVA Factory number"}).text = factoryNumber

        if 'C_GLASS_SHEET1' in position['additional_properties']:
            code = getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET1', 'value')

        product_code_long_name = ''
        sep = '/'
        if glass_unit == 'DGU':
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_THICK_BUILDUP', 'value') + ' '
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET1', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SPACER1', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_AIR_SPACER1', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET2', 'value') + ' '
        elif glass_unit == 'TGU':
            ...
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET2', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SPACER2', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_AIR_SPACER2', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET3', 'value') + ' '
        elif glass_unit == 'QGU':
            ...
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET3', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SPACER3', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_AIR_SPACER3', 'value') + sep
            product_code_long_name = product_code_long_name + \
                getAdditionalPropertiesValue(
                    position, 'C_GLASS_SHEET4', 'value') + ' '
        else:
        ...
        product_code_long_name = product_code_long_name + 'U=' + \
            getAdditionalPropertiesValue(
                position, 'C_GLASS_U_VALUE', 'value') + ' '
        product_code_long_name = product_code_long_name + 'Lt=' + \
            getAdditionalPropertiesValue(
                position, 'C_GLASS_LT_VALUE', 'name') + ' '
        product_code_long_name = product_code_long_name + 'G=' + \
            getAdditionalPropertiesValue(position, 'C_GLASS_G_VALUE', 'value')

        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "309", "comment": "Label: DVA Glass code long text descrition"}).text = product_code_long_name

        dimensions = ''
        dimensions = dimensions + \
            str(dovista_int2int(getAdditionalPropertiesValue(
                position, 'C_GLASS_WIDTH', 'value')))+'x'
        dimensions = dimensions + \
            str(dovista_int2int(getAdditionalPropertiesValue(
                position, 'C_GLASS_HEIGHT', 'value')))+'x'
        #glass_thick_list = str(getAdditionalPropertiesValue(position,'C_GLASS_THICK_BUILDUP','value')).split('-')
        glass_thick_list = [
            dovista_float(getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET1_THICKNESS', 'name')),
            dovista_float(getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET2_THICKNESS', 'name')),
            dovista_float(getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET3_THICKNESS', 'name')),
            dovista_float(getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET4_THICKNESS', 'name')),
            dovista_float(getAdditionalPropertiesValue(
                position, 'C_GLASS_SHEET5_THICKNESS', 'name')),
            dovista_float(getAdditionalPropertiesValue(
        ...
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "310", "comment": "Label: DVA Dimmensions(width,height,thickness)"}).text = dimensions

        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "311", "comment": "Label: DVA Drawing 1"}).text = getAdditionalPropertiesValue(position, 'C_DRAWING', 'value')
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "312", "comment": "Label: DVA Glazing bar elev. glass"}).text = getAdditionalPropertiesValue(position, 'C_GLZBAR_G_ELEV', 'name')
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "313", "comment": "Label: DVA Glazing bar var. glass"}).text = getAdditionalPropertiesValue(position, 'C_GLZBAR_G_VAR', 'name')
        # 14 Emalit colour
        # ET.SubElement(xml_position,'additionalInfo',attrib={"type": "314"}).text = ''
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "316", "comment": "Label: DVA Barcode on label"}).text = orderNumberByCustomer+str(position['order_position'])
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "317", "comment": "Label/Frame: C_GLASS_CODE"}).text = getAdditionalPropertiesValue(position, 'C_GLASS_CODE', 'value')
        production_order = ''
        matchObj = re.match(r'[^\/]*', str(position['vendor_info']))
        if matchObj is not None:
            production_order = matchObj.group(0)
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "318", "comment": "Frame: production order"}).text = production_order
        production_order2 = ''
        matchObj = re.match(r'[^\-]*', str(position['vendor_info']))
        if matchObj is not None:
            production_order2 = matchObj.group(0)
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "319", "comment": "Delivery doc: production order2"}).text = production_order2

        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "109", "comment": "Prints: Opis struktury według klienta"}).text = product_code_long_name

        # klucze dla OrderConfirmation
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "320", "comment": "OrderConfirmation: DVA Purchase order position "}).text = str(position['order_position'])
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "321", "comment": "OrderConfirmation: DVA Glass code long text descrition"}).text = product_code_long_name
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "322", "comment": "OrderConfirmation: BuyersItemIdentification"}).text = str(position['buyers_item_identification'])

        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "330", "comment": "Label/Frame: DVA width"}).text = str(
            dovista_int2int(getAdditionalPropertiesValue(position, 'C_GLASS_WIDTH', 'value')))
        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "331", "comment": "Label/Frame: DVA height"}).text = str(
            dovista_int2int(getAdditionalPropertiesValue(position, 'C_GLASS_HEIGHT', 'value')))
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "332", "comment": "Label: DVA thickness"}).text = str(total_thickness)
        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "333", "comment": "Frame: U-Value"}).text = str(
            getAdditionalPropertiesValue(position, 'C_GLASS_U_VALUE', 'value'))
        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "334", "comment": "Frame: G-Value"}).text = str(
            getAdditionalPropertiesValue(position, 'C_GLASS_G_VALUE', 'value'))
        ET.SubElement(xml_position, 'additionalInfo', attrib={"type": "335", "comment": "Frame: Lt-Value"}).text = str(
            getAdditionalPropertiesValue(position, 'C_GLASS_LT_VALUE', 'value'))
        sideinout_text = 'THIS SIDE IN'
        if factoryNumber in ['T1', 'T2', 'T3', 'T4']:
            sideinout_text = 'THIS SIDE OUT'
        ET.SubElement(xml_position, 'additionalInfo', attrib={
                      "type": "336", "comment": "Label: Side In/Out"}).text = sideinout_text
